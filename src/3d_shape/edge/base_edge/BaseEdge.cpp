...
bool BaseEdge::isVisibleToCamera(
  ...
  if(
    shapeTransformX >= cameraLocation.getX() - (appScreenWidth/2) &&
    shapeTransformX <= cameraLocation.getX() + (appScreenWidth/2)
  ){
    cameraVector.setX(0);
  }else
  {
    cameraVector.setX(shapeTransformX);
  }
  if(
    shapeTransformY >= cameraLocation.getY() - (appScreenHeight/2) &&
    shapeTransformY <= cameraLocation.getY() + (appScreenHeight/2)
  ){
    cameraVector.setY(0);
  }else
  {
    cameraVector.setY(shapeTransformY);
  }
